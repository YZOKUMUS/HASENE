...
class AudioGenerator {
    ...
    playBackgroundMusic(type = 'ambient') {
        ...
        this.stopAllMusic(); // Stop current music
        
        const musicTypes = {
            ambient: this.playAmbientMusic.bind(this),
            focus: this.playFocusMusic.bind(this),
            celebration: this.playCelebrationMusic.bind(this)
        };
    ...
    playAmbientMusic() {
        ...
        const chords = [
            [baseFreq, baseFreq * 1.25, baseFreq * 1.5], // Am
            [baseFreq * 1.125, baseFreq * 1.125 * 1.25, baseFreq * 1.125 * 1.5], // Em
            [baseFreq * 0.75, baseFreq * 0.75 * 1.25, baseFreq * 0.75 * 1.5], // F
            [baseFreq * 1.33, baseFreq * 1.33 * 1.25, baseFreq * 1.33 * 1.5] // G
        ];
        ...
        const playChord = () => {
            ...
            chord.forEach((freq, i) => {
                setTimeout(() => {
                    this.createTone(freq, 3, 'sine', 0.05);
                }, i * 100);
            });
    ...
    playFocusMusic() {
        ...
        const playNote = () => {
            ...
            this.createTone(freq, 0.5, 'triangle', 0.1);
    ...
    playCelebrationMusic() {
        ...
        melody.forEach((freq, i) => {
            setTimeout(() => {
                this.createTone(freq, 0.3, 'sawtooth', 0.2);
            }, i * 150);
        });
        ...
        setTimeout(() => {
            melody.forEach((freq, i) => {
                setTimeout(() => {
                    this.createTone(freq * 1.25, 0.3, 'sine', 0.15);
                }, i * 150);
            });
        }, 600);
    }
    ...
    setMasterVolume(volume) {
        ...
        localStorage.setItem('hasene_master_volume', this.masterVolume.toString());
    }
    
    getMasterVolume() {
        if (this.masterVolume === undefined) {
            this.masterVolume = parseFloat(localStorage.getItem('hasene_master_volume') || '0.7');
        }
    ...
    setMusicEnabled(enabled) {
        ...
        localStorage.setItem('hasene_music_enabled', enabled.toString());
    ...
    isMusicEnabled() {
        if (this.musicEnabled === undefined) {
            this.musicEnabled = localStorage.getItem('hasene_music_enabled') !== 'false';
        }
        return this.musicEnabled;
    }
    
    // 🎤 Enhanced Achievement Sounds with Rarity
    playRarityAchievementSound(rarity) {
        if (!this.audioContext) return;
        
        const rarityEffects = {
            bronze: () => {
                this.createTone(523.25, 0.2, 'sine', 0.3);
                setTimeout(() => this.createTone(659.25, 0.2, 'sine', 0.3), 200);
            },
            silver: () => {
                this.createTone(659.25, 0.15, 'triangle', 0.4);
                setTimeout(() => this.createTone(783.99, 0.15, 'triangle', 0.4), 150);
                setTimeout(() => this.createTone(987.77, 0.2, 'triangle', 0.4), 300);
            },
            gold: () => {
                this.createTone(783.99, 0.1, 'sawtooth', 0.5);
                setTimeout(() => this.createTone(987.77, 0.1, 'sawtooth', 0.5), 100);
                setTimeout(() => this.createTone(1174.7, 0.1, 'sawtooth', 0.5), 200);
                setTimeout(() => this.createTone(1396.9, 0.3, 'sine', 0.4), 300);
            },
            diamond: () => {
                // Epic diamond achievement sound
                const frequencies = [1046.5, 1174.7, 1318.5, 1568, 1760, 2093];
                frequencies.forEach((freq, i) => {
                    setTimeout(() => {
                        this.createTone(freq, 0.15, 'square', 0.6);
                        this.createTone(freq * 0.5, 0.15, 'sine', 0.3);
                    }, i * 80);
                });
                ...
                setTimeout(() => {
                    for (let i = 0; i < 10; i++) {
                        setTimeout(() => {
                            this.createTone(2000 + Math.random() * 1000, 0.05, 'sine', 0.2);
                        }, i * 50);
                    }
                }, 500);
            }
        ...
        const effect = rarityEffects[rarity] || rarityEffects.bronze;
        effect();
    }
    
    // 🎊 Daily Mission Complete Sound
    playDailyMissionComplete() {
        if (!this.audioContext) return;
        
        // Islamic prayer call inspired melody
        const melody = [659.25, 698.46, 783.99, 880, 987.77];
        melody.forEach((freq, i) => {
            setTimeout(() => {
                this.createTone(freq, 0.4, 'sine', 0.3);
            }, i * 200);
        });
        ...
        setTimeout(() => {
            melody.forEach((freq, i) => {
                setTimeout(() => {
                    this.createTone(freq * 0.5, 0.6, 'triangle', 0.1);
                }, i * 200 + 100);
            });
        }, 300);
    }
